...
<div class="min-h-screen bg-base-200 px-4 py-8">
	<div class="mx-auto max-w-3xl">
		...
		<div class="card bg-base-100 shadow-xl overflow-hidden">
			...
			<div class="h-32 bg-gradient-to-r {config.bgGradient} relative">
				<div class="absolute inset-0 flex items-center justify-center opacity-20">
					<config.headerIcon class="h-24 w-24" />
				</div>
			</div>

			<div class="card-body relative -mt-16">
				<!-- Profile Picture (overlapping the banner) -->
				<div class="flex justify-center mb-4">
					<div class="avatar">
						<div
							class="h-32 w-32 rounded-full ring-4 {config.ringColor} ring-offset-4 ring-offset-base-100 bg-base-100"
						>
				...
				{#if !editing}
					...
					<div class="text-center">
						...
						{#if hasProfile && data.profile}
							<h1 class="text-3xl font-bold mb-2">{data.profile.name}</h1>
							<p class="text-base-content/70 mb-4 max-w-xl mx-auto">
								{data.profile.description}
							</p>
						{:else if canEdit}
							<h1 class="text-3xl font-bold mb-2 text-base-content/50">Profile Incomplete</h1>
							<div class="alert alert-info max-w-md mx-auto mb-4">
						...
						<!-- Account Type Badge -->
						<div class="badge {config.badgeStyle} badge-lg gap-2 mb-4">
							<config.icon class="size-4 " />
							{config.label}
						</div>

						<!-- Email -->
						<p class="text-sm text-base-content/60 mb-6">{data.account.email}</p>
						...
						<div class="mt-8 flex flex-wrap justify-center gap-3">
							{#if canEdit}
								<button onclick={startEditing} class="btn btn-primary gap-2">
						...
						>
							...
							<div class="form-control">
								...
								<textarea
									...
									class="textarea textarea-bordered textarea-lg h-40"
									required
									disabled={saving}
								></textarea>
								<label class="label">
