...
<!-- Countdown Timer: Before Pickup Window -->
	...
	>
		...
		{#if browser && 'Notification' in window && Notification.permission === 'default'}
			<button
				...
				class="btn mt-6 w-full gap-2 btn-outline btn-sm btn-primary"
			>
...
<!-- Countdown Timer: During Pickup Window -->
{#if data.reservation.status === 'active' && timeRemaining.isPickupTime && !data.pendingInviteForUser}
	...
	<div
		class="animate-pulse rounded-2xl border-2 border-success bg-linear-to-br from-success/20 to-success/10 p-6 shadow-xl"
	>
		<div class="flex items-center gap-4">
			<div class="flex h-16 w-16 items-center justify-center rounded-full bg-success/20">
				<IconCheckmark class="size-10 text-success" />
			</div>
			<div class="flex-1">
				<h2 class="text-2xl font-bold text-success">🎉 Pickup Time!</h2>
				<p class="mt-1 text-sm text-success/80">Your food is ready to be collected now</p>
			</div>
		</div>
	</div>
...
{#if data.pendingInviteForUser}
	<div class="rounded-2xl border-l-4 border-primary bg-primary/10 p-6">
		<div class="mb-6 text-center">
			...
			<p class="mt-2 text-sm text-primary/80">Someone wants to pass this food collection to you</p>
		</div>
